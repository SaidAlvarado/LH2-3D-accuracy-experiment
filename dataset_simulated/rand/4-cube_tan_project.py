...
elevation_b = np.arctan2( p_b[1],  p_b[2])
...
# pts_a = np.array([np.tan(azimuth_a),       # horizontal pixel  
#                   np.tan(elevation_a) * 1/np.cos(azimuth_a)]).T  # vertical   pixel 

# pts_b = np.array([np.tan(azimuth_b),       # horizontal pixel 
...
E, mask  =  cv2.findEssentialMat(pts_a, pts_b, K, cv2.FM_LMEDS)
